...
class custom_dataset(Dataset):
    def __init__(self, data, normalize=True, Moving=True, Padding=False):
    ...
    def __getitem__(self,idx):       
        X_main, Y_main = self.X_train[0,idx], self.y_train[0,idx]
        X_main = X_main.transpose()
        
        Y_main = Y_main.squeeze()
        
        X_time_main, Y_time_main = self.X_time[0,idx], self.y_time[0,idx]
        ...
        X_end_ind =  np.argmin(abs(X_time_main - (t_sel+t_gap) ))

        X = X[X_st_ind:X_end_ind]
        # X_time = X_time_main[X_st_ind:X_end_ind]
        
        Y_st_ind =  np.argmin(abs(Y_time_main - t_sel))
        Y_end_ind =  np.argmin(abs(Y_time_main - (t_sel+t_gap) ))
        
        Y = Y_main[Y_st_ind:Y_end_ind]
        # Y_time = Y_time_main[Y_st_ind:Y_end_ind]       
